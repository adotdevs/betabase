/**
 * InvestAndMigrate Chatbot Widget - Standalone Script
 * 
 * Usage on https://investandmigrate.com/:
 * <script>
 *   window.ChatbotConfig = {
 *     apiUrl: 'https://api.betabase.pro/api/v1',
 *     adminEmail: 'info@investandmigrate.com',
 *     botAvatar: 'https://www.betabase.pro/img/bot.jpg',
 *     assistantKey: 'investandmigrate'
 *   };
 * </script>
 * <script src="https://www.betabase.pro/chatbot-widget-investandmigrate.js"></script>
 */
...
(function() {
  ...
  const ADMIN_EMAIL = config.adminEmail || 'info@investandmigrate.com';
  const ASSISTANT_KEY = config.assistantKey || 'investandmigrate';
  const SESSION_STORAGE_KEY = 'chatbot_sessionId_investandmigrate';
  const PRIMARY_COLOR = '#1a2e44';
  const SECONDARY_COLOR = '#2c4a6b';
  // Get the script's directory to construct the bot avatar path
  const scriptTag = document.currentScript || document.querySelector('script[src*="chatbot-widget-investandmigrate.js"]');
  ...
  const getAssistantPayload = () => ({ assistantKey: ASSISTANT_KEY });
      ...
      try {
        if (!sessionId) {
          ...
          if (sessionResponse && sessionResponse.success) {
            ...
            localStorage.setItem(SESSION_STORAGE_KEY, sessionResponse.sessionId);
          }
        }
                  ...
                  React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '4px', flex: 1 } },
                    ...
                    React.createElement('div', {
                      style: {
                        ...
                        background: isUser ? ('linear-gradient(135deg, ' + PRIMARY_COLOR + ' 0%, ' + SECONDARY_COLOR + ' 100%)') : '#f0f0f0',
  ...
  loadDependencies(function() {
    ...
    root.id = 'investandmigrate-chatbot-root';
