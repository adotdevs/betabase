...
export function MainComponent() {
	...
	const getTransactionsForCoin = (coinSymbol, transactions) => {
		...
		return completedTransactions.reduce((acc, transaction) => acc + transaction.amount, 0);
	};
	const getEuroCryptoBalance = () => {
		if (!UserData?.additionalCoins || !UserData?.transactions) return 0;
		const euroCoin = UserData.additionalCoins.find(
			(coin) => String(coin.coinName || '').toLowerCase() === 'euro'
		);
		if (!euroCoin) return 0;
		return getTransactionsForCoin(euroCoin.coinName, UserData.transactions);
	};
	const openEuroWithdraw = () => {
		Navigate('/assets', { state: { assetsTab: 'fiat', openEuroWithdraw: true } });
	};
								...
								<div className="card-body p-3 py-0">
									<div className="table-responsive">
										<div className="d-flex flex-wrap gap-2 mb-2 px-1">
											<button
												type="button"
												className={assetsTab === 'crypto' ? 'btn btn-primary btn-rounded btn-sm' : 'btn btn-outline-primary btn-rounded btn-sm'}
											...
											<button
												type="button"
												className={assetsTab === 'fiat' ? 'btn btn-primary btn-rounded btn-sm' : 'btn btn-outline-primary btn-rounded btn-sm'}
