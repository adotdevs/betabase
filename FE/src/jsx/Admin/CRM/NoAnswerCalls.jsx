...
const NoAnswerCalls = () => {
    ...
    const [noAnswerCalls, setNoAnswerCalls] = useState([]);
            ...
            try {
                ...
                if (me?.user?.role === 'admin') {
                    ...
                    if (!updated?.adminPermissions?.canAccessCallDashboard) {
                        toast.error('Access denied to No Answer Calls');
    ...
    // OPTIMIZED: Fetch no-answer calls - removed unnecessary statistics fetch
    const fetchNoAnswerCalls = useCallback(async () => {
        try {
            setRefreshing(true);
            const params = {
            ...
            const noAnswerRes = await getCompletedCallsApi(params);
            
            console.log('🔍 [fetchNoAnswerCalls] API Response:', {
                success: noAnswerRes.success,
                callsCount: noAnswerRes.calls?.length || 0,
                firstCall: noAnswerRes.calls?.[0] ? {
                    id: noAnswerRes.calls[0]._id,
                    hasMetadata: !!noAnswerRes.calls[0].metadata,
                    metadata: noAnswerRes.calls[0].metadata,
                    hasLogs: !!noAnswerRes.calls[0].logs,
                    logsLength: noAnswerRes.calls[0].logs?.length || 0,
                    hasEndedReason: !!noAnswerRes.calls[0].endedReason
                } : null
            });

            if (noAnswerRes.success) {
                setNoAnswerCalls(noAnswerRes.calls || []);
                // Use backend pagination data (more accurate)
                if (noAnswerRes.pagination) {
                    setPagination(prev => ({
                        ...prev,
                        total: noAnswerRes.pagination.total || 0,
                        totalPages: noAnswerRes.pagination.totalPages || 1
                    }));
                }
            }
        } catch (error) {
            console.error('Error fetching no-answer calls:', error);
            toast.error('Error loading no-answer calls');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    }, [noAnswerCalls, logsEndedReasons]);
    ...
    const getEndedReasonColor = (endedReason) => {
        ...
        // No answer cases
        if (reason.includes('did not answer') || reason.includes('no answer') || reason.includes('no-answer') || reason.includes('customer-did-not-answer')) {
            return 'default';
        }
        // Customer busy
    ...
    const handleDeleteCall = async (callId, deleteLead = false) => {
        try {
            ...
            const callToDelete = noAnswerCalls.find(c => c._id === callId);
    ...
    const handleDeleteAllNoAnswerCalls = async () => {
        if (pagination.total === 0) {
            toast.warning('No no-answer calls to delete');
            return;
        }

        if (!window.confirm(`Are you sure you want to delete all ${pagination.total} no-answer call(s)? This action cannot be undone.`)) {
            return;
        }

        try {
            setDeletingByStatus('no-answer');
            const response = await deleteCallsByStatusApi('no-answer');
            if (response.success) {
                const deletedCount = response.deletedCount || 0;
                toast.success(`Successfully deleted ${deletedCount} no-answer call(s)`);
                fetchNoAnswerCalls();
            } else {
                toast.error(response.message || 'Failed to delete no-answer calls');
            }
        } catch (error) {
            console.error('Error deleting all no-answer calls:', error);
            toast.error(error.response?.data?.msg || 'Failed to delete no-answer calls');
        } finally {
            setDeletingByStatus(null);
        }
    };

    // Filter no-answer calls by search term
    const filteredCalls = noAnswerCalls.filter(call => {
            ...
            if (successful > 0) {
                ...
                if (deleteLeads && isSuperAdmin) {
                    ...
                    selectedCalls.forEach(callId => {
                        const call = noAnswerCalls.find(c => c._id === callId);
                        if (call?.leadId) {
                            const leadId = typeof call.leadId === 'object' ? call.leadId._id || call.leadId : call.leadId;
            ...
            >
                {loading ? (
                    ...
                    <Box sx={{ p: 3 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                                ...
                                <Button
                                    ...
                                    color="warning"
                                    size="small"
                                    startIcon={deletingByStatus === 'no-answer' ? <CircularProgress size={16} /> : <DeleteIcon />}
                                    onClick={handleDeleteAllNoAnswerCalls}
                                    disabled={deletingByStatus === 'no-answer' || pagination.total === 0}
                                >
                                    {deletingByStatus === 'no-answer' ? 'Deleting...' : 'Delete All'}
                                </Button>
                                <Button
                                    variant="outlined"
                                    startIcon={refreshing ? <CircularProgress size={16} /> : <Refresh />}
                                    onClick={fetchNoAnswerCalls}
                                    disabled={refreshing}
                                >
                                    Refresh
                                </Button>
                        ...
                        {/* No Answer Calls Table */}
                        {filteredCalls.length > 0 ? (
                            <Card elevation={3} sx={{ borderRadius: 3, border: '2px solid', borderColor: 'warning.light', bgcolor: 'warning.50' }}>
                                <CardContent>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                        <Typography variant="body2" color="text.secondary">
                                            {pagination.total} no-answer call(s) found
                                        </Typography>
                                        <Chip
                                            label="Customer Did Not Answer"
                                            color="warning"
                                            size="small"
                                        ...
                                        <Table size="small">
                                            <TableHead>
                                                <TableRow sx={{ bgcolor: 'warning.50' }}>
                                                    ...
                                                    <TableCell sx={{ fontWeight: 700, fontSize: '0.875rem' }}>Initiated At</TableCell>
                                                    <TableCell sx={{ fontWeight: 700, fontSize: '0.875rem' }}>Ended At</TableCell>
                                                ...
                                                {filteredCalls.map((call) => {
                                                    ...
                                                    <TableRow key={call._id} hover>
                                                        ...
                                                        <TableCell>
                                                            <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem' }}>
                                                                {formatDateTime(derived.startedAt)}
                                                            </Typography>
                                                        </TableCell>
                                                        <TableCell>
                                                            <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem' }}>
                            ...
                            <Card elevation={3} sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
                                <CardContent>
                                    <Box sx={{ textAlign: 'center', py: 8 }}>
                                        <PhoneMissed sx={{ fontSize: 64, color: 'text.secondary', mb: 2 }} />
                                        <Typography variant="h6" color="text.secondary" gutterBottom>
                                            {searchTerm ? 'No Matching No Answer Calls' : 'No No Answer Calls'}
                                        </Typography>
                                        <Typography variant="body2" color="text.secondary">
                                            {searchTerm ? 'Try adjusting your search criteria' : 'All calls have been answered or no calls have been missed yet'}
                                        </Typography>
                                    </Box>
                                </CardContent>
                            </Card>
                        ...
                        >
                            ...
                            <DialogContent>
                                <Typography sx={{ mb: 2 }}>
                                    Are you sure you want to delete this no-answer call? This action cannot be undone.
                                </Typography>
                                {(() => {
                                    const callToDelete = noAnswerCalls.find(c => c._id === deleteCallConfirm);
