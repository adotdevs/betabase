...
const CallDashboard = () => {
    ...
    const [callBackFilter, setCallBackFilter] = useState(null); // null = all, true = call back, false = non call back
    ...
    const fetchDashboardData = useCallback(async () => {
        try {
            ...
            if (completedRes?.success) {
                // DEBUG: Log raw API response to check structured outputs structure
                if (completedRes.calls && completedRes.calls.length > 0) {
                    const sampleCall = completedRes.calls[0];
                    console.log('🔍 [fetchDashboardData] Raw Completed Calls API Response:', {
                        totalCalls: completedRes.calls.length,
                        sampleCall: {
                            id: sampleCall._id,
                            hasMetadata: !!sampleCall.metadata,
                            metadataKeys: sampleCall.metadata ? Object.keys(sampleCall.metadata) : [],
                            hasArtifact: !!sampleCall.metadata?.artifact,
                            artifactKeys: sampleCall.metadata?.artifact ? Object.keys(sampleCall.metadata.artifact) : [],
                            hasStructuredOutputs: !!sampleCall.metadata?.structuredOutputs,
                            structuredOutputsKeys: sampleCall.metadata?.structuredOutputs ? Object.keys(sampleCall.metadata.structuredOutputs) : [],
                            fullMetadata: sampleCall.metadata,
                            fullCall: sampleCall // Log entire call object to see structure
                        }
                    });
                }
        ...
        try {
            ...
            while (hasMorePages) {
                ...
                if (callBackFilter !== null) {
                    params.callBack = callBackFilter;
                }
                const response = await getCompletedCallsApi(params);
    ...
    }, [summaryPagination.limit, callBackFilter]);
    ...
    const fetchSummariesPage = useCallback(async (page = 1, limit = 10) => {
        try {
            ...
            if (response.success && response.calls) {
                // DEBUG: Log raw API response to check structured outputs structure
                console.log('🔍 [fetchSummariesPage] Raw API Response:', {
                    totalCalls: response.calls.length,
                    firstCall: response.calls[0] ? {
                        id: response.calls[0]._id,
                        hasMetadata: !!response.calls[0].metadata,
                        metadataKeys: response.calls[0].metadata ? Object.keys(response.calls[0].metadata) : [],
                        hasArtifact: !!response.calls[0].metadata?.artifact,
                        artifactKeys: response.calls[0].metadata?.artifact ? Object.keys(response.calls[0].metadata.artifact) : [],
                        fullMetadata: response.calls[0].metadata,
                        fullCall: response.calls[0] // Log entire call object to see structure
                    } : null
                });
                
                // #region agent log
                fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:589',message:'API response received',data:{totalCalls:response.calls.length,callBackValues:response.calls.slice(0,5).map(c=>({id:c._id,callBack:c.callBack,hasMetadata:!!c.metadata,hasStructuredOutputs:!!c.metadata?.structuredOutputs})),callBackFilter},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                // #endregion
    ...
    }, [callsWithSummaries, summaryPagination.limit, loadingSummaries, fetchSummariesPage, callBackFilter]);

    // Refetch summaries when callBackFilter changes (but not on initial mount)
    ...
    useEffect(() => {
        ...
        // Clear existing summaries and reset to first page when filter changes
        setCallsWithSummaries([]);
        setSummaryPagination(prev => ({ ...prev, page: 1, total: 0, totalPages: 1 }));
        setSummaryCarouselIndex(0);
        ...
        const fetchPage = async () => {
            try {
                ...
                const params = {
                    page: 1,
                    limit: summaryPagination.limit * 2,
                    status: 'completed',
                    includeTranscript: 'true'
                };
                if (callBackFilter !== null) {
                ...
                const response = await getCompletedCallsApi(params);
                if (response.success && response.calls) {
                    const summaries = response.calls.filter(call => call.summary && call.summary.trim().length > 0);
                    setCallsWithSummaries(summaries);
                    setSummaryPagination(prev => ({
                        ...prev,
                        total: response.pagination?.total || summaries.length,
                        totalPages: response.pagination?.totalPages || 1
                    }));
                }
            } catch (error) {
                console.error('Error fetching summaries with filter:', error);
            } finally {
                setLoadingSummaries(false);
            }
        };
        fetchPage();
    }, [callBackFilter, summaryPagination.limit]); // Only trigger when filter changes
    ...
    const getStructuredOutputs = (call) => {
        ...
        try {
            ...
            if (structuredOutputs && typeof structuredOutputs === 'object') {
                // Handle the format: { "uuid": { "name": "...", "result": ..., "compliancePlan": {...} } }
                const outputs = [];
                for (const [key, value] of Object.entries(structuredOutputs)) {
                    if (value && typeof value === 'object') {
                        // Preserve all properties from the structured output, not just name and result
                        outputs.push({
                            id: key,
                            name: value.name || key,
                            result: value.result,
                            // Preserve all other properties (like compliancePlan, etc.)
                            ...Object.keys(value).reduce((acc, prop) => {
                                if (prop !== 'name' && prop !== 'result') {
                                    acc[prop] = value[prop];
                                }
                                return acc;
                            }, {})
    ...
    // Extract callBack flag from call data, with fallback to extract from structured outputs
    const getCallBackFromCall = (call) => {
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1295',message:'getCallBackFromCall - entry',data:{callId:call?._id,callBack:call?.callBack,metadataCallBack:call?.metadata?.callBack,hasStructuredOutputs:!!call?.metadata?.structuredOutputs},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        // #endregion
        
        // First check if callBack is already set
        if (call.callBack !== undefined) {
            // #region agent log
            fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1298',message:'getCallBackFromCall - returning call.callBack',data:{callId:call?._id,result:call.callBack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            // #endregion
            return call.callBack;
        }
        if (call.metadata?.callBack !== undefined) {
            // #region agent log
            fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1300',message:'getCallBackFromCall - returning metadata.callBack',data:{callId:call?._id,result:call.metadata.callBack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            // #endregion
            return call.metadata.callBack;
        }
        
        // Fallback: Extract from structured outputs
        const structuredOutputs = call.metadata?.structuredOutputs || 
                                 call.metadata?.artifact?.structuredOutputs ||
                                 null;
        if (structuredOutputs) {
            // #region agent log
            fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1305',message:'getCallBackFromCall - checking structured outputs',data:{callId:call?._id,structuredOutputsEntries:Object.entries(structuredOutputs).map(([k,v])=>({key:k,name:v?.name,result:v?.result}))},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
            // #endregion
            ...
            for (const [key, output] of Object.entries(structuredOutputs)) {
                if (output && typeof output === 'object') {
                    ...
                    if ((outputName.includes('appointment') || outputName.includes('callback') || outputName.includes('call back')) && 
                        output.result === true) {
                        // #region agent log
                        fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1310',message:'getCallBackFromCall - found appointment in structured outputs',data:{callId:call?._id,outputName,result:output.result},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
                        // #endregion
                        return true;
                    }
                }
        ...
        // #region agent log
        fetch('http://127.0.0.1:7243/ingest/0b7ae174-df61-4914-8ec0-ed47b5d9d381',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'CallDashboard.jsx:1315',message:'getCallBackFromCall - returning false',data:{callId:call?._id},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        // #endregion
        return false;
    };
    const currentSummaryCallBack = currentSummaryCall ? getCallBackFromCall(currentSummaryCall) : false;
                    ...
                    <Box sx={{ p: 3 }}>
                        ...
                        <Card elevation={0} sx={{ 
                            ...
                            <CardContent sx={{ p: 3 }}>
                                ...
                                {/* Header Section with Title, Call Back Filter, and Navigation */}
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mb: 2 }}>
                                    {/* Top Row: Title, Call Back Filter, and Navigation */}
                                    <Box sx={{ 
                                        display: 'flex', 
                                        ...
                                        borderRadius: 2,
                                        background: (theme) => theme.palette.mode === 'dark'
                                            ? 'linear-gradient(135deg, rgba(25, 25, 35, 0.8) 0%, rgba(15, 15, 25, 0.9) 100%)'
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1, flexWrap: 'wrap' }}>
                                            ...
                                            {/* Call Back Filter Buttons in Top Bar */}
                                            <Box sx={{ display: 'flex', gap: 0.5, ml: 1, flexWrap: 'wrap' }}>
                                                <Button
                                                    size="small"
                                                    onClick={async () => {
                                                        setCallBackFilter(null);
                                                        setCallsWithSummaries([]);
                                                        setSummaryPagination(prev => ({ ...prev, page: 1, total: 0, totalPages: 1 }));
                                                        setSummaryCarouselIndex(0);
                                                        setLoadingSummaries(true);
                                                    ...
                                                    sx={{ 
                                                        ...
                                                        py: 0.5,
                                                        fontSize: '0.75rem',
                                                        fontWeight: callBackFilter === null ? 700 : 500,
                                                        textTransform: 'none',
                                                        background: callBackFilter === null 
                                                            ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'
                                                            : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)',
                                                        color: callBackFilter === null ? 'white' : 'text.primary',
                                                        boxShadow: callBackFilter === null ? '0 2px 8px rgba(102, 126, 234, 0.3)' : 'none',
                                                        '&:hover': {
                                                            background: callBackFilter === null 
                                                ...
                                                <Button
                                                    size="small"
                                                    startIcon={<PhoneCallback sx={{ fontSize: 14 }} />}
                                                    onClick={async () => {
                                                        setCallBackFilter(true);
                                                        setCallsWithSummaries([]);
                                                        setSummaryPagination(prev => ({ ...prev, page: 1, total: 0, totalPages: 1 }));
                                                        setSummaryCarouselIndex(0);
                                                        setLoadingSummaries(true);
                                                        try {
                                                            ...
                                                            console.error('Error fetching call back summaries:', error);
                                                        } finally {
                                                            setLoadingSummaries(false);
                                                        }
                                                    }}
                                                    sx={{ 
                                                        ...
                                                        py: 0.5,
                                                        fontSize: '0.75rem',
                                                        fontWeight: callBackFilter === true ? 700 : 500,
                                                        textTransform: 'none',
                                                        background: callBackFilter === true 
                                                            ? 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)'
                                                            : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)',
                                                        color: callBackFilter === true ? 'white' : 'text.primary',
                                                        boxShadow: callBackFilter === true ? '0 2px 8px rgba(17, 153, 142, 0.3)' : 'none',
                                                        '&:hover': {
                                                            background: callBackFilter === true 
                                                ...
                                                <Button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        try {
                                                            ...
                                                            console.error('Error fetching non call back summaries:', error);
                                                        } finally {
                                                            setLoadingSummaries(false);
                                                        }
                                                    }}
                                                    sx={{ 
                                                        ...
                                                        py: 0.5,
                                                        fontSize: '0.75rem',
                                                        fontWeight: callBackFilter === false ? 700 : 500,
                                                        textTransform: 'none',
                                                        background: callBackFilter === false 
                                                            ? 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)'
                                                            : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)',
                                                        color: callBackFilter === false ? 'white' : 'text.primary',
                                                        boxShadow: callBackFilter === false ? '0 2px 8px rgba(240, 147, 251, 0.3)' : 'none',
                                                        '&:hover': {
                                                            background: callBackFilter === false 
                                        ...
                                        <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'center', flexWrap: 'wrap' }}>
                                            {/* Delete Button */}
                                            {callsWithSummaries.length > 0 && callsWithSummaries[summaryCarouselIndex] && (
                                            ...
                                            {summaryPagination.totalPages > 1 && (
                                                <Box sx={{ 
                                                    ...
                                                    <Typography variant="body2" sx={{ minWidth: 80, textAlign: 'center', fontWeight: 600, fontSize: '0.75rem', color: 'text.primary' }}>
                                                        {summaryPagination.page} / {summaryPagination.totalPages}
                                                    </Typography>
                                            ...
                                            {callsWithSummaries.length > 0 && (
                                                <Tooltip title="Reload All Summaries">
                                                    ...
                                                    >
                                                        {loadingSummaries ? <CircularProgress size={16} /> : <Refresh sx={{ fontSize: 18 }} />}
                                                    </IconButton>
                                                </Tooltip>
                                            )}
                                            {/* Item Navigation */}
                                    ...
                                    {/* Prominent Call Back Status Display */}
                                    {callsWithSummaries.length > 0 && callsWithSummaries[summaryCarouselIndex] && (
                                        <Box sx={{ 
                                            display: 'flex', 
                                            justifyContent: 'center', 
                                            ...
                                            <Chip
                                                icon={currentSummaryCallBack ? <PhoneCallback sx={{ fontSize: 18 }} /> : null}
                                                label={currentSummaryCallBack ? 'CALL BACK REQUIRED' : 'NO CALL BACK'}
                                                sx={{
                                                    height: 36,
                                                    fontSize: '0.85rem',
                                                    fontWeight: 700,
                                                    ...
                                                    py: 0.5,
                                                    border: 'none',
                                                    background: currentSummaryCallBack 
                                                        ? 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)'
                                                        : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                                                    color: 'white',
                                                    boxShadow: currentSummaryCallBack 
                                ...
                                {loadingSummaries ? (
                                    ...
                                    <Box>
                                        {/* Call Details and Summary/Transcript Side by Side */}
                                        <Grid container spacing={2} sx={{ mb: 1.5 }}>
                                            {/* Call Details with Lead Information */}
                                            <Grid item xs={12} md={5}>
                                                <Box sx={{ 
                                                    ...
                                                    '&:hover': {
                                                        boxShadow: '0 8px 30px rgba(17, 153, 142, 0.2)',
                                                        ...
                                                        <Box sx={{ mb: 1.5, pb: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                                                                ...
                                                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                                                    <Typography variant="body1" sx={{ fontWeight: 700, mb: 0.25, fontSize: '0.9rem' }}>
                                                                        {typeof callsWithSummaries[summaryCarouselIndex].leadId === 'object'
                                                    ...
                                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                                        {currentSummaryEndedReason && (
                                                            <Box sx={{ 
                                                                ...
                                                                <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Ended Reason:</Typography>
                                                                <Chip
                                                                    label={currentSummaryEndedReason}
                                                                    color={getEndedReasonColor(currentSummaryEndedReason)}
                                                                    size="small"
                                                        ...
                                                        <Box sx={{ 
                                                            ...
                                                            <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Duration:</Typography>
                                                        ...
                                                        <Box sx={{ 
                                                            ...
                                                            <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Completed:</Typography>
                                                            <Typography variant="body2" sx={{ fontWeight: 700, color: 'text.primary', fontSize: '0.85rem' }}>
                                                        ...
                                                        {currentSummaryStartedAt && (
                                                            <Box sx={{ 
                                                                ...
                                                                <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Started:</Typography>
                                                                <Typography variant="body2" sx={{ fontWeight: 700, color: 'text.primary', fontSize: '0.85rem' }}>
                                                        ...
                                                        <Box sx={{ 
                                                            ...
                                                            p: 1.5,
                                                            background: currentSummaryCallBack 
                                                                ? 'linear-gradient(135deg, rgba(17, 153, 142, 0.2) 0%, rgba(56, 239, 125, 0.15) 100%)'
                                                                : 'linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.1) 100%)',
                                                            borderRadius: 2,
                                                            border: '1px solid',
                                                            borderColor: currentSummaryCallBack ? 'rgba(17, 153, 142, 0.5)' : 'rgba(102, 126, 234, 0.3)',
                                                            boxShadow: currentSummaryCallBack 
                                                                ? '0 2px 8px rgba(17, 153, 142, 0.2)'
                                                            ...
                                                            <Typography variant="body2" sx={{ 
                                                                ...
                                                                <PhoneCallback sx={{ 
                                                                    fontSize: 16, 
                                                                    color: currentSummaryCallBack ? '#11998e' : '#667eea'
                                                                }} />
                                                                Call Back:
                                                            </Typography>
                                                            <Chip
                                                                label={currentSummaryCallBack ? 'YES' : 'NO'}
                                                                sx={{ 
                                                                    fontWeight: 800, 
                                                                    fontSize: '0.8rem',
                                                                    height: 28,
                                            ...
                                            <Grid item xs={12} md={7}>
                                                <Box sx={{ 
                                                    ...
                                                    {/* Toggle Buttons */}
                                                    <Box sx={{ display: 'flex', gap: 1, mb: 1.5, justifyContent: 'flex-start' }}>
                                                        <Button
                                                            size="small"
                                                            ...
                                                            sx={{
                                                                ...
                                                                fontSize: '0.8rem',
                                                                borderRadius: 2,
                                                                background: activeContentView === 'summary'
                                                                    ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'
                                                                    : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)',
                                                                color: activeContentView === 'summary' ? 'white' : 'text.primary',
                                                                boxShadow: activeContentView === 'summary' 
                                                            ...
                                                            <span>
                                                                <Button
                                                                    ...
                                                                    sx={{
                                                                        ...
                                                                        fontSize: '0.8rem',
                                                                        borderRadius: 2,
                                                                        background: activeContentView === 'transcript'
                                                                            ? 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)'
                                                                            : (theme) => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)',
                                                                        color: activeContentView === 'transcript' ? 'white' : 'text.primary',
                                                                        boxShadow: activeContentView === 'transcript' 
                                                    ...
                                                    {activeContentView === 'summary' ? (
                                                        <Box sx={{ flex: 1, p: 2, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider', minHeight: 200, maxHeight: 400, overflow: 'auto' }}>
                                                    {/* Structured Outputs - MOST IMPORTANT DATA - Display First */}
                                                    {currentSummaryStructuredOutputs.length > 0 && (
                                                        <Box sx={{ mb: 2, pb: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
                                                            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, color: 'primary.main', display: 'flex', alignItems: 'center', gap: 0.75, fontSize: '0.9rem' }}>
                                                                <CheckCircle sx={{ fontSize: 18 }} />
                                                                Structured Outputs
                                                            </Typography>
                                                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                                                                {currentSummaryStructuredOutputs.map((output, idx) => {
                                                                    // Get all additional properties (excluding id, name, result)
                                                                    const additionalProps = Object.keys(output).filter(key => 
                                                                        !['id', 'name', 'result'].includes(key)
                                                                    );
                                                                    ...
                                                                    return (
                                                                        ...
                                                                        >
                                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1, mb: additionalProps.length > 0 ? 1.5 : 0 }}>
                                                                                ...
                                                                                {typeof output.result === 'boolean' ? (
                                                                                    ...
                                                                                    <Chip
                                                                                        label={String(output.result).length > 50 ? String(output.result).substring(0, 50) + '...' : String(output.result)}
                                                                                        color="default"
                                                                                        sx={{
                                                                                            fontWeight: 700,
                                                                                            fontSize: '0.875rem',
                                                                                    ...
                                                                                    {additionalProps.map((prop) => {
                                                                                        ...
                                                                                        return (
                                                                                            <Box key={prop} sx={{ mb: 1 }}>
                                                                                                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', display: 'block', mb: 0.5 }}>
                                                                            ...
                                                                            {/* Show full result if it's a long string */}
                                                    ...
                                                    <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1.5, color: 'primary.main', fontSize: '0.9rem' }}>
                                                        Call Summary
                                                    </Typography>
                                                    <Typography
                                                        variant="body2"
                                                    ...
                                                    {typeof callsWithSummaries[summaryCarouselIndex].transcript === 'string' ? (
                                                        ...
                                                        <Typography variant="body2" color="text.secondary" sx={{ fontStyle: 'italic' }}>
                                                            ...
                                                            <Box sx={{ flex: 1, p: 2, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider', minHeight: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
