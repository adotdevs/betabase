 ...
 *     //   buttonBackground: '#023c5b',
 *     //   sendButtonBackground: '#023c5b',
 *     //   userBubbleBackground: '#023c5b'
 *     // },
 *     //
 *     // Assistant selection (all optional)
 *     // useWidgetAssistant: true, // default is true for backwards compatibility
 *     // assistantId: 'asst_...'    // optional override if backend supports it
 *   };
 * </script>
 * <script src="https://your-domain.com/chatbot-widget.js"></script>
 */
...
(function() {
  ...
  const THEME = (config && typeof config.theme === 'object' && config.theme) ? config.theme : {};
  const ASSISTANT_ID = typeof config.assistantId === 'string' && config.assistantId.trim().length > 0 ? config.assistantId.trim() : null;
  const USE_WIDGET_ASSISTANT = typeof config.useWidgetAssistant === 'boolean' ? config.useWidgetAssistant : true;

  const getAssistantSelectorPayload = () => {
    const payload = { useWidgetAssistant: USE_WIDGET_ASSISTANT };
    if (ASSISTANT_ID) payload.assistantId = ASSISTANT_ID;
    return payload;
  };

  const getThemeValue = (key, fallback) => {
    const val = THEME && Object.prototype.hasOwnProperty.call(THEME, key) ? THEME[key] : undefined;
  ...
  const ChatBot = function() {
    ...
    const styles = {
      chatButton: {
        ...
        height: '60px',
        borderRadius: '50%',
        background: getThemeValue('buttonBackground', getThemeValue('primary', '#023c5b')),
        border: 'none',
        cursor: 'pointer',
        display: 'flex',
        alignItems: 'center',
      ...
      chatHeader: {
        background: getThemeValue('headerBackground', getThemeValue('primary', '#023c5b')),
        padding: '5px 15px',
        display: 'flex',
      ...
      sendButton: {
        background: getThemeValue('sendButtonBackground', getThemeValue('primary', '#023c5b')),
        border: 'none',
        padding: '10px',
        borderRadius: '50%',
        cursor: 'pointer',
      ...
      dialogButtonConfirm: {
        padding: '10px 20px',
        border: 'none',
        background: getThemeValue('primary', '#023c5b'),
        color: 'white',
        borderRadius: '6px',
        cursor: 'pointer',
        fontSize: '14px',
                  ...
                  React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '4px', flex: 1 } },
                    ...
                    React.createElement('div', {
                      style: {
                        ...
                        background: isUser ? getThemeValue('userBubbleBackground', getThemeValue('primary', '#023c5b')) : '#f0f0f0',
